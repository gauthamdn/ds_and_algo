...
public class L733_floodfill_bfs {
	...
	public static int[][] floodFill(int[][] image, int sr, int sc, int targetcolor) {
		...
		Queue<pair_class> q = new LinkedList<>();
		image[sr][sc] = targetcolor;
		q.add(new pair_class(sr,sc));
		
		while(!q.isEmpty()) {
			
			pair_class pair = q.remove();
			int r = pair.first;
			int c = pair.second;
			...
			for(int k = 0;k<4;k++) {
				...
				if(isSafe(newi, newj, n,m) && image[newi][newj] == sourceColor) {
					...
					q.add(new pair_class(newi,newj));
				}
...
// had to name pair_class due to collision from another file 
class pair_class{
	int first;
	int second;
	
	pair_class(int first, int second){
		this.first = first;
		this.second = second;
	}
}
